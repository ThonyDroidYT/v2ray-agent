...
checkSystem() {
	...
	if [[ -z ${release} ]]; then
		echo "Esta secuencia de comandos no es compatible con este sistema, informe el siguiente registro al desarrollador"
		cat /etc/issue
		cat /proc/version
		exit 0
	fi
...
initVar() {
	...
	# La dirección del nodo CDN
	add=

	# Progreso total de la instalación
	totalProgress=1
	...
	# Método de instalación personalizado actual 01234
	...
	# La lógica de certificado de renovación integrada ya no usa un script separado - RenewTLS
	renewTLS=$1
}
...
readInstallType() {
	...
	# Verifique el directorio de instalación
	if [[ -d "/etc/v2ray-agent" ]]; then
		# Método de instalación de detección v2ray-core
...
# Verifique el directorio de archivos y la ruta de acceso
	...
	if [[ -n "${coreInstallType}" ]]; then
		if [[ "${coreInstallType}" == 1 ]]; then
			if [[ -n $(pgrep -f xray/xray) ]]; then
				echoContent yellow "\nNúcleo: Xray-core [en funcionamiento]"
			else
				echoContent yellow "\nNúcleo: Xray-core [no se está ejecutando]"
			fi

		elif [[ "${coreInstallType}" == 2 || "${coreInstallType}" == 3 ]]; then
			if [[ -n $(pgrep -f v2ray/v2ray) ]]; then
				echoContent yellow "\nCore: v2ray-core [en funcionamiento]"
			else
				echoContent yellow "\nNúcleo: v2ray-core [no se está ejecutando]"
			fi
		fi
		# Leer el tipo de protocolo
		readInstallProtocolType

		if [[ -n ${currentInstallProtocolType} ]]; then
			echoContent yellow "Acuerdo instalado：\c"
		fi
...
# Inicializar el directorio de instalación
mkdirTools() {
...
# Kit de instalacion
installTools() {
	echoContent skyBlue "\nCalendario  $1/${totalProgress} : Herramienta de instalación"
	if [[ "${release}" == "centos" ]]; then
		echoContent green " ---> Verifique e instale jq, nginx epel source, yum-utils, semanage"
		# jq epel源
	...
	# Solucionar problemas individuales del sistema en ubuntu
	...
	echoContent green " ---> Verifique e instale actualizaciones [Las máquinas nuevas serán lentas, espere con paciencia]"
	...
	if ! find /usr/bin /usr/sbin | grep -q -w wget; then
		echoContent green " ---> Instalar wget"
	...
	if ! find /usr/bin /usr/sbin | grep -q -w unzip; then
		echoContent green " ---> instalar unzip"
	...
	if ! find /usr/bin /usr/sbin | grep -q -w tar; then
		echoContent green " ---> Instalar tar"
	...
	if ! find /usr/bin /usr/sbin | grep -q -w nginx; then
		echoContent green " ---> Instalar nginx"
		if [[ "${centosVersion}" == "8" ]]; then
			rpm -ivh ${nginxEpel} >/etc/v2ray-agent/error.log 2>&1
		else
	...
	# para apagar el firewall

	if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
		echoContent green " ---> instalar acme.sh"
		curl -s https://get.acme.sh | sh >/etc/v2ray-agent/tls/acme.log
		if [[ ! -d "$HOME/.acme.sh" ]] || [[ -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
			echoContent red "  la instalación de acme falló--->"
			echoContent yellow "Solución de problemas:"
			echoContent red "  1.No se pudo obtener el archivo Github, espere a que Gitub se restaure e intente, se puede ver el progreso de la restauración [https://www.githubstatus.com/]"
			echoContent red "  2.acme.sh Hay un error en el script, puedes verlo. [https://github.com/acmesh-official/acme.sh] issues"
			exit 0
		fi
	fi
}

# Inicializar la configuración del certificado de la aplicación Nginx
initTLSNginxConfig() {
	handleNginx stop
	echoContent skyBlue "\nCalendario  $1/${totalProgress} : Inicializar la configuración del certificado de la aplicación Nginx"
	if [[ -n "${currentHost}" ]]; then
		echo
		read -r -p "Leer el último registro de instalación, si desea utilizar el nombre de dominio de la última instalación? [y/n]:" historyDomainStatus
		if [[ "${historyDomainStatus}" == "y" ]]; then
			domain=${currentHost}
			echoContent yellow "\n ---> nombre de dominio：${domain}"
		else
			echo
			echoContent yellow "Ingrese el nombre de dominio a configurar：www.v2ray-agent.com --->"
			read -r -p "Nombre de dominio: 》 " domain
		fi
	else
		echo
		echoContent yellow "Ingrese el nombre de dominio a configurar：www.v2ray-agent.com --->"
	...
	if [[ -z ${domain} ]]; then
		echoContent red "  El nombre de dominio no puede estar vacío--->"
		initTLSNginxConfig
	else
		...
		# Iniciar nginx
		handleNginx start
		echoContent yellow "\nCompruebe si la IP está configurada en el VPS actual"
		checkIP
		# Prueba nginx
		echoContent yellow "\nCompruebe si normalmente se accede a Nginx"
		sleep 0.5
		domainResult=$(curl -s "${domain}/test" | grep fjkvymb6len)
		if [[ -n ${domainResult} ]]; then
			handleNginx stop
			echoContent green "\n --->  La configuración de Nginx es exitosa"
		else
			echoContent red " ---> No se puede acceder al servidor normalmente, verifique si el nombre de dominio es correcto, la resolución DNS del nombre de dominio y la configuración del firewall son correctos--->"
			exit 0
		fi
	fi
}

# Modificar la configuración de redireccionamiento de nginx
...
# Ver el estado del certificado TLS
checkTLStatus() {
...
# Instalar V2Ray, versión especificada
installV2Ray() {
	readInstallType
...
xrayVersionManageMenu() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "1.Potenciar"
	echoContent yellow "2.Regresa"
	echoContent yellow "3.Cerrar Xray-core"
	echoContent yellow "4.Abrir Xray-core"
	echoContent yellow "5.Reiniciar Xray-core"
	echoContent red "=============================================================="
	read -r -p "Por favor elige: 》 " selectXrayType
	if [[ "${selectXrayType}" == "1" ]]; then
		updateXray
	elif [[ "${selectXrayType}" == "2" ]]; then
		echoContent yellow "\n1.Debido a las frecuentes actualizaciones de Xray-core, solo se pueden revertir las dos últimas versiones"
		echoContent yellow "2.No hay garantía de que se pueda usar normalmente después de la reversión."
		echoContent yellow "3.Si la versión revertida no es compatible con la configuración actual, no podrá conectarse, opere con precaución"
		echoContent skyBlue "------------------------Version-------------------------------"
		...
		echoContent skyBlue "--------------------------------------------------------------"
		read -r -p "Ingrese la versión que desea revertir: " selectXrayVersionType
		...
		if [[ -n "${version}" ]]; then
			...
			echoContent red "\n ---> La entrada es incorrecta, vuelva a ingresar"
...
updateV2Ray() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		...
		# Usar versión bloqueada
		if [[ -n "${v2rayCoreVersion}" ]]; then
			version=${v2rayCoreVersion}
		fi
		echoContent green " ---> versión v2ray-core: ${version}"
		...
		echoContent green " ---> Versión actual de v2ray-core: $(/etc/v2ray-agent/v2ray/v2ray --version | awk '{print $2}' | head -1)"
		...
		if [[ -n "$1" ]]; then
			read -r -p "La versión de reversión es ${version}, ¿desea continuar? [y/n]: " rollbackV2RayStatus
			if [[ "${rollbackV2RayStatus}" == "y" ]]; then
				if [[ "${coreInstallType}" == "2" || "${coreInstallType}" == "3" ]]; then
					echoContent green " ---> Versión actual de v2ray-core: $(/etc/v2ray-agent/v2ray/v2ray --version | awk '{print $2}' | head -1)"
				elif [[ "${coreInstallType}" == "1" ]]; then
					echoContent green " ---> Versión actual de Xray-core: $(/etc/v2ray-agent/xray/xray --version | awk '{print $2}' | head -1)"
				fi
				...
				echoContent green " ---> Abandonar la versión alternativa"
			fi
		elif [[ "${version}" == "v$(/etc/v2ray-agent/v2ray/v2ray --version | awk '{print $2}' | head -1)" ]]; then
			read -r -p "La versión actual es la misma que la última. ¿Quieres reinstalarla? [y/n]:" reInstallV2RayStatus
			if [[ "${reInstallV2RayStatus}" == "y" ]]; then
				handleV2Ray stop
				rm -f /etc/v2ray-agent/v2ray/v2ray
				rm -f /etc/v2ray-agent/v2ray/v2ctl
				...
				echoContent green " ---> Abandonar la reinstalación"
			fi
		else
			read -r -p "La última versión es: ${version}，Ya sea para actualizar？[y/n]：" installV2RayStatus
			if [[ "${installV2RayStatus}" == "y" ]]; then
				rm -f /etc/v2ray-agent/v2ray/v2ray
				rm -f /etc/v2ray-agent/v2ray/v2ctl
				updateV2Ray
			else
				echoContent green " ---> Abandonando actualización"
			fi
...
# Actualizar Trojan-Go
updateTrojanGo() {
	echoContent skyBlue "\nCalendario  $1/${totalProgress} : Actualizar Trojan-Go"
	if [[ ! -d "/etc/v2ray-agent/trojan/" ]]; then
		echoContent red " ---> No se detecta el directorio de instalación, ejecute el script para instalar el contenido"
		menu
		exit 0
	fi
	if find /etc/v2ray-agent/trojan/ | grep -q "trojan-go"; then
		version=$(curl -s https://github.com/p4gefau1t/trojan-go/releases | grep /trojan-go/releases/tag/ | head -1 | awk -F "[/]" '{print $6}' | awk -F "[>]" '{print $2}' | awk -F "[<]" '{print $1}')
		echoContent green " ---> Versión Trojan-Go: ${version}"
		...
		echoContent green " ---> Versión actual de Trojan-Go: $(/etc/v2ray-agent/trojan/trojan-go --version | awk '{print $2}' | head -1)"
		if [[ -n $(/etc/v2ray-agent/trojan/trojan-go --version) ]]; then
			...
			if [[ "${version}" == "$(/etc/v2ray-agent/trojan/trojan-go --version | awk '{print $2}' | head -1)" ]]; then
				read -r -p "La versión actual es la misma que la última. ¿Quieres reinstalarla? [y/n]:" reInstalTrojanGoStatus
				...
				read -r -p "La última versión es: ${version}，¿Está actualizado? [y/n]：" installTrojanGoStatus
				if [[ "${installTrojanGoStatus}" == "y" ]]; then
					rm -rf /etc/v2ray-agent/trojan/trojan-go
					updateTrojanGo 1
				else
					echoContent green " ---> Abandonar la instalación"
				fi
			fi
		fi
	fi
...
# Verifique que todo el servicio esté disponible
checkGFWStatue() {
	readInstallType
...
# V2Ray se inicia automáticamente después de arrancar
installV2RayService() {
	echoContent skyBlue "\nCalendario  $1/${totalProgress} : Configure V2Ray para que se inicie automáticamente después de arrancar"
...
EOF
		...
		echoContent green " ---> Configure V2Ray para que se inicie correctamente después de arrancar"
	fi
}

# Xray se inicia automáticamente después de arrancar
installXrayService() {
...
# Trojan se inicia automáticamente después de arrancar
installTrojanService() {
	echoContent skyBlue "\nValendario $1/${totalProgress} : Configurar Trojan para que se inicie automáticamente después de arrancar"
	if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
...
EOF
		...
		echoContent green " ---> Configurar Trojan para que se inicie correctamente después de arrancar"
	fi
}
# Operar V2Ray
handleV2Ray() {
...
# manageUser 用户管理
manageUser() {
	echoContent skyBlue "\nCalendario $1/${totalProgress} : Gestión multiusuario"
	echoContent skyBlue "-----------------------------------------------------"
	echoContent yellow "1.Agregar usuario"
	echoContent yellow "2.Eliminar usuarios"
	echoContent skyBlue "-----------------------------------------------------"
	read -r -p "Por favor elige: 》 " manageUserType
	if [[ "${manageUserType}" == "1" ]]; then
		addUser
	elif [[ "${manageUserType}" == "2" ]]; then
		removeUser
	else
		echoContent red " ---> selección incorrecta"
	fi
}

# 自定义uuid
customUUID() {
	read -r -p "Ya sea para personalizar UUID ？[y/n]:" customUUIDStatus
	echo
	if [[ "${customUUIDStatus}" == "y" ]]; then
		read -r -p "Por favor ingrese el UUID:" currentCustomUUID
		echo
		if [[ -z "${currentCustomUUID}" ]]; then
			echoContent red " ---> UUID no puede estar vacío"
		else
			local repeat=
			...
			if [[ -f "/tmp/v2ray-agent" && -n $(cat /tmp/v2ray-agent) ]]; then
				echoContent red " ---> El UUID no se puede repetir"
				rm /tmp/v2ray-agent
				exit 0
			fi
		fi
...
# 自定义email
customUserEmail() {
	read -r -p "Ya sea para personalizar el correo electrónico ？[y/n]:" customEmailStatus
	echo
	if [[ "${customEmailStatus}" == "y" ]]; then
		read -r -p "Por favor introduzca una dirección de correo electrónico válida:" currentCustomEmail
		echo
		if [[ -z "${currentCustomEmail}" ]]; then
			echoContent red " ---> el correo electrónico no puede estar vacío"
		else
			local repeat=
			...
			if [[ -f "/tmp/v2ray-agent" && -n $(cat /tmp/v2ray-agent) ]]; then
				echoContent red " ---> El correo electrónico no se puede repetir"
				rm /tmp/v2ray-agent
				exit 0
			fi
		fi
...
addUser() {
	...
	echoContent yellow "Después de agregar un nuevo usuario，Necesito revisar la suscripción"
	read -r -p "Ingrese el número de usuarios que desea agregar：" userNum
	echo
	if [[ -z ${userNum} || ${userNum} -le 0 ]]; then
		echoContent red " ---> La entrada es incorrecta, vuelva a ingresar"
		exit 0
	fi
	...
	reloadCore
	echoContent green " ---> Agregar completo "
	showAccounts 1
}
...
removeUser() {
	...
	if echo ${currentInstallProtocolType} | grep -q 0; then
		...
		read -r -p "Seleccione el número de usuario que desea eliminar [solo se admite la eliminación única]:" delUserIndex
...
# 更新脚本
updateV2RayAgent() {
	echoContent skyBlue "\ncalendario  $1/${totalProgress} : Actualizar el script v2ray-agent"
	...
	if wget --help | grep -q show-progress; then
		...
		wget -c -q --show-progress -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/ThonyDroidYT/v2ray-agent/master/install_es.sh"
	else
		#wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/mack-a/v2ray-agent/master/install.sh"
		wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/ThonyDroidYT/v2ray-agent/master/install_es.sh"
	fi
	...
	echoContent yellow " ---> Ejecute [vasma] manualmente para abrir el script."
	echoContent green " ---> versión actual: ${version}\n"
	echoContent yellow "Si la actualización no se realiza correctamente, ejecute el siguiente comando manualmente\n"
	echoContent skyBlue "wget -P /root -N --no-check-certificate "https://raw.githubusercontent.com/ThonyDroidYT/v2ray-agent/master/install_es.sh" && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
...
# 流媒体工具箱
streamingToolbox() {
	echoContent skyBlue "\nCaracterísticas 1/${totalProgress} : Caja de herramientas de transmisión de medios"
	echoContent red "\n=============================================================="
	echoContent yellow "1.Detección de Netflix"
	echoContent yellow "2.Cualquier máquina de aterrizaje de puerta para desbloquear Netflix"
	echoContent yellow "3.DNS desbloquea la transmisión\n"
	read -r -p "Por favor elige: 》" selectType
...
# 任意门解锁netflix
dokodemoDoorUnblockNetflix() {
	echoContent skyBlue "\nCaracterísticas 1/${totalProgress} : Cualquier máquina de aterrizaje de puerta para desbloquear Netflix"
	echoContent red "\n=============================================================="
	echoContent yellow "# Precauciones"
	echoContent yellow "Para obtener detalles sobre cómo desbloquear cualquier puerta, consulte este artículo. [https://github.com/mack-a/v2ray-agent/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"

	echoContent yellow "1.Agregar saliente"
	echoContent yellow "2.Agregar entrante"
	echoContent yellow "3.Desinstalar"
	read -r -p "Por favor elija: 》" selectType
...
# 设置任意门解锁Netflix【出站】
setDokodemoDoorUnblockNetflixOutbounds() {
	read -r -p "Ingrese la IP para desbloquear Netflix vps: " setIP
	if [[ -n "${setIP}" ]]; then
		cat <<EOF >${configPath}10_ipv4_outbounds.json
{
  "outbounds": [
...
EOF
		reloadCore
		echoContent green " ---> Agrega Netflix para jugar y desbloquear con éxito"
		echoContent yellow " ---> Nodos relacionados que no admiten trojan"
		exit 0
	fi
	echoContent red " ---> ip no puede estar vacío"
}

# 设置任意门解锁Netflix【入站】
setDokodemoDoorUnblockNetflixInbounds() {
	read -r -p "Ingrese la IP que tiene permiso para acceder a los vps de Netflix desbloqueados:" setIP
	if [[ -n "${setIP}" ]]; then
...
EOF
		reloadCore
		echoContent green " ---> Agregue la máquina de aterrizaje a la entrada y desbloquee Netflix con éxito"
		echoContent yellow " ---> Los nodos relacionados del trojan no admiten esta operación"
		exit 0
	fi
	echoContent red " ---> ip no puede estar vacío"
}

# Retire cualquier puerta para desbloquear Netflix
removeDokodemoDoorUnblockNetflix() {
...
EOF
	...
	reloadCore
	echoContent green " ---> Desinstalado exitosamente"
}
...
# Compruebe si vps es compatible con Netflix
checkNetflix() {
	echoContent red "\nPrecauciones"
	echoContent yellow " 1.Solo puede detectar si vps es compatible con Netflix"
	echoContent yellow " 2.No se puede detectar si el proxy es compatible con Netflix después del desbloqueo de dns"
	echoContent yellow " 3.Puede detectar si Netflix es compatible después de desbloquear la configuración de vps dns\n"
	echoContent skyBlue " ---> comprobación"
	netflixResult=$(curl -s -m 2 https://www.netflix.com | grep "Not Available")
	if [[ -n ${netflixResult} ]]; then
		echoContent red " ---> Netflix no está disponible"
		exit 0
	fi
	...
	echoContent skyBlue " ---> Comprueba si se puede jugar a Breaking Bad"
	result=$(curl -s -m 2 https://www.netflix.com/title/70143836 | grep "page-404")
	if [[ -n ${result} ]]; then
		echoContent green " ---> Solo puedo ver dramas caseros"
		exit 0
	fi
	echoContent green " ---> Desbloqueo de Netflix"
	exit 0
}

# dns desbloquea Netflix
dnsUnlockNetflix() {
	echoContent skyBlue "\nCaracterísticas 1/${totalProgress} : DNS para desbloquear Netflix"
	echoContent red "\n=============================================================="
	echoContent yellow "1.añadir"
...
# 设置dns
setUnlockDNS() {
	read -r -p "Ingrese el DNS para desbloquear Netflix:" setDNS
	if [[ -n ${setDNS} ]]; then
...
EOF
		...
		echoContent green "\n ---> El desbloqueo de DNS se agregó correctamente, esta configuración no es válida para Trojan-Go"
		echoContent yellow "\n ---> Si aún no puede mirar, puede probar las siguientes dos soluciones"
		echoContent yellow " 1.Reiniciar vps"
		echoContent yellow " 2.Después de desinstalar dns y desbloquearlo, modifique la configuración local [/etc/resolv.conf] DNS y reinicie vps\n"
	else
		echoContent red " ---> dns no puede estar vacío "
	fi
	exit 0
}

# Eliminar el desbloqueo de Netflix
removeUnlockDNS() {
	cat <<EOF >${configPath}11_dns.json
{
	"dns": {
...
# instalación personalizada v2ray-core
customV2RayInstall() {
	echoContent skyBlue "\n========================Instalación personalizada============================"
	echoContent yellow "VLESS frente, debe instalar 0, si solo necesita instalar 0, presione Enter"
	if [[ "${selectCoreType}" == "2" ]]; then
	...
	read -r -p "Seleccione [Opción múltiple], [por ejemplo: 123]:" selectCustomInstallType
	echoContent skyBlue "--------------------------------------------------------------"
...
# Elija la instalación principal: v2ray-core, xray-core, versión bloqueada de v2ray-core [xtls]
selectCoreInstall() {
	echoContent skyBlue "\nCaracterísticas 1/${totalProgress} : Elija la instalación principal"
	echoContent red "\n=============================================================="
	echoContent yellow "1.Xray-core"
	echoContent yellow "2.v2ray-core"
	echoContent yellow "3.v2ray-core[XTLS]"
	echoContent red "=============================================================="
	read -r -p "Por favor elige: 》 " selectCoreType
	...
	"3")
		...
		;;
	*)
		echoContent red ' ---> Elige mal, elige de nuevo'
		selectCoreInstall
		;;
	esac
}

# instalación de v2ray-core
v2rayCoreInstall() {
	cleanUp xrayClean
	selectCustomInstallType=
	totalProgress=17
...
coreVersionManageMenu() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		echoContent red "\n ---> No se detecta el directorio de instalación, ejecute el script para instalar el contenido"
		menu
		exit 0
	fi
	if [[ "${coreInstallType}" == "1" ]]; then
...
# Certificado de inspección de tareas programadas
cronRenewTLS() {
...
manageAccount() {
	echoContent skyBlue "\nCaracterísticas 1/${totalProgress} : Administración de cuentas"
	echoContent red "\n=============================================================="
	echoContent yellow "# Eliminar cada vez、Después de agregar una cuenta，Necesita revisar las suscripciones para generar suscripciones\n"
	echoContent yellow "1.Ver cuenta"
	echoContent yellow "2.Ver suscripción"
	echoContent yellow "3.Agregar usuario"
	echoContent yellow "4.Eliminar usuarios"
	echoContent red "=============================================================="
	read -r -p "Por favor escribe: 》" manageAccountStatus
...
subscribe() {
	if [[ -n "${configPath}" ]]; then
		...
		echoContent yellow "# La suscripción se regenerará al ver la suscripción"
		echoContent yellow "# Cada vez que agregas、Eliminar la cuenta requiere volver a verificar la suscripción"
		...
		if [[ -n $(ls /etc/v2ray-agent/subscribe) ]]; then
			ls /etc/v2ray-agent/subscribe | while read -r email; do
				...
				echoContent yellow "Código QR en línea：https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=https://${currentHost}/s/${email}\n"
				echo "https://${currentHost}/s/${email}" | qrencode -s 10 -m 1 -t UTF8
				echoContent skyBlue "--------------------------------------------------------------"
			done
		fi
	else
		echoContent red " ---> No instalado"
	fi
}

# Instalar MT
setMTG() {
	echoContent skyBlue "\nCaracterísticas 1/${totalProgress} : Configurar MTPROTO[FAKE TLS]"
	echoContent skyBlue "-------------------------Observaciones---------------------------------"
	echoContent yellow "# El uso de MTPROTO tiene el riesgo de ser bloqueado, familiarícese con los riesgos"
	echoContent yellow "# Permita el acceso al puerto 8443\n"
	echoContent yellow "1.Añadir"
	echoContent yellow "2.Desinstalar"
	echoContent yellow "3.Ver cuenta"
	read -r -p "Por favor elija: 》 " setMTGStatus
	if [[ "${setMTGStatus}" == "1" ]]; then
		echoContent skyBlue " ---> Descarga MTG"
		installMTG
		echoContent skyBlue " ---> Generar MTPROTO FAKE TLS "
		initMTGSecret
		echoContent skyBlue " ---> Instale MTG para que se inicie automáticamente después de arrancar"
		installMTGService
		handleMTG start
		showMTGAccount
...
unInstallMTG() {
	...
	echoContent green " ---> Desinstalación completa"
	exit 0
}

# Ver la información de la cuenta MTG
showMTGAccount() {
	local ip=$(curl -s https://api.ip.sb/ip --ipv4)
	if [[ -z ${ip} ]]; then
		ip=$(curl -s ipinfo.io/ip --ipv4)
		if [[ -z ${ip} ]]; then
			echoContent red " ---> la adquisición de ip falló, ingrese manualmente"
		fi
	fi
	echoContent skyBlue "========================= TGEnlace =============================\n"
...
menu() {
	...
	echoContent green "versión actual：v2.4.16"
	echoContent green "Github：https://github.com/mack-a/v2ray-agent"
	echoContent green "Descripción: Script de coexistencia siete en uno\c"
	showInstallStatus
	echoContent red "\n=============================================================="
	if [[ -n "${coreInstallType}" ]]; then
		echoContent yellow "1.Reinstalar"
	else
		echoContent yellow "1.Instalación"
	fi

	echoContent yellow "2.Instalar en cualquier combinación"
	echoContent skyBlue "-------------------------Gestión de herramientas-----------------------------"
	echoContent yellow "3.Administración de cuentas"
	echoContent yellow "4.Reemplazar la estación de camuflaje"
	echoContent yellow "5.Renovar certificado"
	echoContent yellow "6.Reemplazar modo CDN"
	echoContent yellow "7.Descarga de IPv6"
	echoContent yellow "8.Herramientas de transmisión"
	echoContent yellow "9.Agregar nuevo puerto"
	echoContent skyBlue "-------------------------Gestión de versiones-----------------------------"
	echoContent yellow "10.Gestión core central"
	echoContent yellow "11.Actualizar Trojan-Go"
	echoContent yellow "12.Actualizar secuencia de comandos"
	echoContent yellow "13.Instalar BBR, script DD"
	echoContent skyBlue "-------------------------Gestión del Script-----------------------------"
	echoContent yellow "14.Ver registro"
	echoContent yellow "15.Desinstalar secuencia de comandos"
	echoContent red "=============================================================="
	mkdirTools
	aliasInstall
	read -r -p "Por favor elija: 》 " selectInstallType
